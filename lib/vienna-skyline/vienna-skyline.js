...
export function createViennaSkyline(container, userOpts = {}) {
  ...
  let visible = false, raf = 0, last = 0, t = 0, revealStart = -1, ready = false;
  let cssW = 0, cssH = 0;
  ...
  function requestRender() {
    if (reduceMotion) { uniforms.uReveal.value = 1; update(0.016); renderer.render(scene, camera); return; }
  ...
  return {
    ...
    destroy() {
      destroyed = true;
      cancelAnimationFrame(raf); cancelAnimationFrame(resizeRaf); clearInterval(colorTimer);
      io.disconnect();
