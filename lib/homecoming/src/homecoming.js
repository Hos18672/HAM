...
// distant groups wander across it, and groups of birds — V formations and loose flocks —
...
export function createHomecoming(hostEl, options = {}) {
  ...
  const sky = [];
  {
    ...
    SKY.forEach((size, g) => {
      const isV = size <= 15;
      const shape = flockShape(size, isV ? 'v' : 'flock');
  ...
  function launchGroup(G) {
    ...
    const shape = flockShape(n, reduced ? 'flock' : n <= 18 || G.k % 3 !== 1 ? 'v' : 'flock');
  ...
  function ringTarget(j, t) {
    ...
    const ky = kx < 1 ? 1.2 : 1;
    return [Math.cos(a) * (R + wob + ringO[j * 2]) * kx, Math.sin(a) * (R * 0.8 + wob * 0.7 + ringO[j * 2 + 1]) * ky + 0.06];
  }
  ...
  function step(now) {
    ...
    for (let i = 0; i < N; i++) {
      if (state[i] >= 2) {
        ...
        phase[i] += dt * (open[i] > 0.05 ? 14 + (i % 3) : 0);
      }
      dispX[i] = px[i] + s2 * scatter[i * 2];
      dispY[i] = py[i] + s2 * scatter[i * 2 + 1];
    }
    field.update(dispX, dispY, dt);
    for (let i = 0; i < N; i++) {
      aBird.set([dispX[i], dispY[i], heading[i], phase[i]], i * 4);
    ...
    for (const g of sky) {
      ...
      for (const m of g.members) {
        ...
        if (ringMix > 0) {
          ringA[j] += dt * (0.3 + 0.05 * Math.sin(j)) * ringMix;
