...
export function createPageSky(host, options = {}) {
  ...
  function shape(g) {
    ...
    g.single = n === 1;
    g.depth = Math.random();
    g.v = (g.single ? rand(85, 125) : 80) + 60 * g.depth;
  ...
  // CHANGED FOR THIS SITE: a flock is going somewhere, and knows where.
  // Left to the random walk alone it wanders on the spot, and on a page of
  // cards that is fatal: the no-fly boxes on either side of a gutter cancel
  // each other out in `away`, so a flock can sit in the corridor between two
  // cards under steady pressure and never be pushed anywhere. It spends its
  // life beside the cards instead of crossing the sky. So each one is given
  // a point well past the far edge — simply "keep going the way you set
  // off" — and leans towards it far too weakly to straighten out the wander.
  function aimAcross(g) {
    g.gx = g.x + Math.cos(g.h) * (W + H);
    g.gy = g.y + Math.sin(g.h) * (W + H);
    g.pen = 0;
    g.leaving = false;
  }
  ...
  function step(dt, show) {
    ...
    for (const g of groups) {
      ...
      // CHANGED FOR THIS SITE: lean towards where it is going. An order of
      // magnitude weaker than the pull of a card, so the wander still owns
      // the next second and this only owns the next minute.
      g.h += angDiff(Math.atan2(g.gy - g.y, g.gx - g.x), g.h) * Math.min(1, dt * (g.leaving ? 0.85 : 0.22));
      ...
      // in a moment, and bend the heading away from whatever is close
      {
        ...
        // CHANGED FOR THIS SITE: how long it has been hemmed in. Forgotten
        // about half again as fast as it is earned, so threading a gap on
        // the way through costs nothing and only being stuck counts.
        g.pen = w > 0.22 ? g.pen + dt : Math.max(0, g.pen - dt * 1.5);
      }
      // CHANGED FOR THIS SITE: hemmed in this long, it gives up on this
      // patch of sky rather than going on nosing at the same two cards. It
      // makes for the open air off the top corner it is nearest, quicker
      // than it was flying, and comes back somewhere else in a few seconds.
      if (!g.leaving && g.pen > 1.5) {
        g.leaving = true;
        g.gx = g.x + (g.x < W / 2 ? -1 : 1) * (W * 0.6 + 200);
        g.gy = H + 220;
      }
      const pace = g.v * (g.leaving ? 1.35 : 1) * (1 + 0.18 * Math.sin(time * 0.37 + g.seed) + 0.08 * Math.sin(time * 1.1 + g.seed * 3.1));
      ...
      if (g.x < -m || g.x > W + m || g.y < -m || g.y > H + m) {
        ...
        g.on = false;
        g.wait = rand(2, 7); // CHANGED FOR THIS SITE: they leave more often now
        continue;
      }
      const c = Math.cos(g.h);
      const s = Math.sin(g.h);
